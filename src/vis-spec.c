...
int count_samples (double, double, double);
int rotate_cw (const uint8_t *, uint8_t *, int, int);
int rotate_ccw (const uint8_t *, uint8_t *, int, int);
int bmp (const uint8_t *, int, int);
...
main (void) {
  ...
  int choice, i, j, nlines, *rgb, width, height, count, apply_gamma;
  size_t ibuf, image_bytes;
  double zr, zb, zg, **cmxyz, lambda, *xyzbar;
  double *white_xyz, *rgb_double, **p, *w, **pinv, *coeff, **npm, **npminv;
  ...
  // Determine the number of wavelength samples before allocating image memory.
  height = count_samples (cmxyz[0][0], cmxyz[nlines - 1][0], INTERVAL);
  if (height <= 0) {
    fprintf (stderr, "ERROR: Invalid wavelength range or sampling interval.\n");
    exit (EXIT_FAILURE);
  }
  image_bytes = image_buffer_size (width, height);
  bufferv = allocate_ustrmem (image_bytes);
  bufferh = allocate_ustrmem (image_bytes);
...
*/
  ...
  fprintf (stdout, "\nApply the sRGB transfer function (required for a standard sRGB bitmap)? ");
  memset (temp, 0, MAX_STRINGLEN * sizeof (char));
  inputtext (temp);
  ...
  ibuf = 0u;  // Index of image bufferv
  for (count=0; count<height; count++) {
    lambda = cmxyz[0][0] + ((double) count * INTERVAL);
    if (lambda > cmxyz[nlines - 1][0]) {
      // count_samples() and this expression use the same interval.  This clamp
      // only absorbs a possible final floating-point rounding difference.
      lambda = cmxyz[nlines - 1][0];
    }
...
*/
    ...
    // Clip to the displayable [0,1] range and quantize to 8-bit RGB without
    // converting an out-of-range floating-point value to int.
    for (i=0; i<3; i++) {
      if (rgb_double[i] <= 0.0) {
        rgb[i] = 0;
...
//  fprintf (stdout, "\n");
  ...
  switch (choice) {
    ...
    case 1:
      rotate_cw (bufferv, bufferh, width, height);  // Rotate bitmap 90-deg clockwise.
      break;
    case 2:
      rotate_ccw (bufferv, bufferh, width, height);  // Rotate 90-deg counter-clockwise.
      break;
    default:
  ...
  if (bmp (bufferh, height, width) == EXIT_FAILURE) {
    fprintf (stderr, "ERROR: Unable to write out.bmp.\n");
    exit (EXIT_FAILURE);
  }
...
count_cmf_rows (const char *filename) {
  ...
  while ((status = readline (fi, line, MAX_STRINGLEN)) != -1) {
    ...
    if (parsed > 0) {
      if (count == INT_MAX) {
        fprintf (stderr, "ERROR: Too many CMF rows.\n");
        fclose (fi);
        return (-1);
      }
      count++;
  ...
  if (ferror (fi)) {
    fprintf (stderr, "ERROR: Unable to read color-matching file %s.\n", filename);
    fclose (fi);
    return (-1);
  }
  fclose (fi);
...
// Load validated CMF rows and require strictly increasing wavelengths.
int
...
// Return the number of uniformly spaced wavelength samples.
int
count_samples (double first, double last, double interval) {

  long double ratio, nearest, samples;

  if (!isfinite (first) || !isfinite (last) || !isfinite (interval) ||
      !(last >= first) || !(interval > 0.0)) return (-1);

  ratio = ((long double) last - (long double) first) / (long double) interval;
  nearest = roundl (ratio);
  if (fabsl (ratio - nearest) <=
      ((long double) DBL_EPSILON * 64.0L * fmaxl (1.0L, fabsl (ratio)))) {
    ratio = nearest;
  }
  samples = floorl (ratio) + 1.0L;
  if ((samples < 1.0L) || (samples > (long double) INT_MAX)) return (-1);
  return ((int) samples);
}

// Rotate the displayed positive-height BMP image 90 degrees clockwise.
// Source and destination contain tightly packed BGR triples.
int
rotate_cw (const uint8_t *bufferv, uint8_t *bufferh, int width, int height) {

  int x, y;
  size_t src, dst;

  if ((bufferv == NULL) || (bufferh == NULL) || (width <= 0) || (height <= 0)) {
    return (EXIT_FAILURE);
  }

  for (y=0; y<height; y++) {
    for (x=0; x<width; x++) {
      src = (((size_t) y * (size_t) width) + (size_t) x) * 3u;
      dst = ((((size_t) (width - x - 1) * (size_t) height) + (size_t) y) * 3u);
      bufferh[dst] = bufferv[src];
      bufferh[dst + 1u] = bufferv[src + 1u];
      bufferh[dst + 2u] = bufferv[src + 2u];
    }
  }
...
// Rotate the displayed positive-height BMP image 90 degrees counter-clockwise.
int
rotate_ccw (const uint8_t *bufferv, uint8_t *bufferh, int width, int height) {
  ...
  for (y=0; y<height; y++) {
    for (x=0; x<width; x++) {
      src = (((size_t) y * (size_t) width) + (size_t) x) * 3u;
      dst = ((((size_t) x * (size_t) height) + (size_t) (height - y - 1)) * 3u);
      bufferh[dst] = bufferv[src];
      bufferh[dst + 1u] = bufferv[src + 1u];
      bufferh[dst + 2u] = bufferv[src + 2u];
    }
...
// Create a 24-bit uncompressed BMP output file.
int
bmp (const uint8_t *buffer, int width, int height) {

  size_t row_size, image_size, padding_size, c;
  int x, y;
  uint8_t padding[3] = {0, 0, 0};
  FILE *fo;

  if (buffer == NULL) return (EXIT_FAILURE);
  (void) image_buffer_size (width, height);

  row_size = ((size_t) width * 3u + 3u) & ~(size_t) 3u;
  image_size = row_size * (size_t) height;
  padding_size = row_size - ((size_t) width * 3u);

  errno = 0;
  fo = fopen ("out.bmp", "wbx");
  if (fo == NULL) {
    if (errno == EEXIST) {
      fprintf (stderr, "Output file out.bmp already exists.\n");
    } else {
      fprintf (stderr, "ERROR: Unable to open output file out.bmp.\n");
    }
    return (EXIT_FAILURE);
  }

  write_u16_le (fo, 0x4d42u);
  write_u32_le (fo, (uint32_t) (54u + image_size));
  write_u16_le (fo, 0u);
  write_u16_le (fo, 0u);
  write_u32_le (fo, 54u);
  ...
  for (y=0; y<height; y++) {
    for (x=0; x<width; x++) {
      if ((fputc ((int) buffer[c++], fo) == EOF) ||
          (fputc ((int) buffer[c++], fo) == EOF) ||
          (fputc ((int) buffer[c++], fo) == EOF)) {
        fclose (fo);
        return (EXIT_FAILURE);
      }
    ...
    if ((padding_size > 0u) &&
        (fwrite (padding, 1u, padding_size, fo) != padding_size)) {
      fclose (fo);
      return (EXIT_FAILURE);
    }
  }

  if (fclose (fo) != 0) return (EXIT_FAILURE);
  return (EXIT_SUCCESS);
}
...
// Other choices are general colorimetric whites for comparison; selecting one
// retains the sRGB/BT.709 primaries and transfer function but is no longer a
...
illum_white (double *white_xyz) {
  ...
  if (choice == 0) {
    ...
    fprintf (stdout, "NOTE: this is not the nominal D65 white that defines standard sRGB.\n");
  } else {
...
image_buffer_size (int width, int height) {
  ...
  if ((size_t) height > ((size_t) UINT32_MAX - 54u) / row_size) {
    fprintf (stderr, "ERROR: Image is too large for a classic BMP file.\n");
    exit (EXIT_FAILURE);
  }
